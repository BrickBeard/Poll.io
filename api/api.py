...
def api_polls():
    if request.method == "POST":
        ...
        polls = (
            Topics.query.filter_by(status=1)
            .join(Polls)
...
def api_poll_vote():
    ...
    poll_count = (
        UserPolls.query.filter_by(topic_id=topic.id).filter_by(user_id=user.id).count()
    )
    if poll_count > 0:
...
def api_poll(poll_name):
    ...
    return (
        jsonify({"Polls": [poll.to_json()]})
        if poll
        else jsonify({"message": "poll not found"})
    )
